...
class Part1:
    ...
    def solve(self) -> None:
        ...
        for line in self.lines:
            ...
            assert match
            for _ in range(int(match["num"])):
                self.stack.push_back(match["to"], self.stack.pop(match["from"]))

    def get_result(self) -> str:
        result = ""
        for idx in range(1, self.stack.num_stacks() + 1):
            result += self.stack.pop(str(idx))
        return f"the crates on top of the stacks after CrateMover 9000 rearrangement are: {result}"
