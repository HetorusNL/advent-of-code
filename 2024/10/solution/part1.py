    ...
    def step_path(self, x: int, y: int, current_value: int):
        ...
        for next_x, next_y in self.next_pos(x, y):
            if self.get_pos(next_x, next_y) == next_value:
                if next_value == 9:
                    # if we're at 9, we found a trail head, set it to true
                    self.trail_heads[f"[{next_x}, {next_y}]"] = True
                else:
        ...
        for x in range(self.width):
            for y in range(self.height):
                if self.get_pos(x, y) == 0:
                    # reset the trailhead dict before recursing
                    self.trail_heads: dict[str, bool] = {}
                    self.step_path(x, y, 0)
                    self.result += len(self.trail_heads)
