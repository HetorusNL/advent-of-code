...
class Part2:
    ...
    def solve(self) -> None:
        ...
        for pattern in self.patterns:
            col_reflections, row_reflections = self.solve_pattern(pattern)
            # unsmudge the pattern
            smudge_found = False
            for col_idx in range(len(pattern[0])):
                for row_idx in range(len(pattern)):
                    self.invert(pattern, col_idx, row_idx)
                    new_col_reflections, new_row_reflections = self.solve_pattern(pattern)
                    new_reflections = []
                    for new_col_reflection in new_col_reflections:
                        if new_col_reflection not in col_reflections:
                            new_reflections.append(new_col_reflection)
                    for new_row_reflection in new_row_reflections:
                        if new_row_reflection not in row_reflections:
                            new_reflections.append(new_row_reflection)
                    if new_reflections:
                        smudge_found = True
                        self.result += sum(new_reflections)
                        break
                    # if not found, undo the inversion
                    self.invert(pattern, col_idx, row_idx)
                if smudge_found:
                    break

    def invert(self, pattern: list[str], col_idx: int, row_idx: int):
        char = pattern[row_idx][col_idx]
        new_char = "#" if char == "." else "."
        row = pattern[row_idx]
        start = row[:col_idx]
        end = row[col_idx + 1 :]
        new_row = start + new_char + end
        pattern[row_idx] = new_row
                ...
                if col_idx + 1 < width / 2:
                    ...
                    for test_col_idx in range(col_idx, width):
                        if any(
                            ...
                            is_reflection = False
                if is_reflection:
                    col_reflections.append(col_idx + 1)
        # test row reflection
                ...
                if row_idx + 1 < height / 2:
                    ...
                    for test_row_idx in range(row_idx + 1, height):
                        if pattern[test_row_idx] != pattern[row_idx - (test_row_idx - row_idx) + 1]:
                            ...
                            break
                if is_reflection:
                    row_reflections.append(100 * (row_idx + 1))
        return col_reflections, row_reflections
